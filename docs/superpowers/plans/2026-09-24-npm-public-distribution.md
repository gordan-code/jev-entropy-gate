...
- [x] **Step 5: Commit the isolated build-entry change.**
...
- [x] **Step 1: Add failing tests for package metadata and denylisted content.** Verify metadata is publishable (`private` absent/false, `type: module`, `engines.node >=22.18.0`, `@ast-grep/napi` exactly `0.45.3`), the exact expected entries are allowed, and `.env`, `.env.example`, `helloagents/`, source, tests, reports, configs, source maps, nested secrets, and extra path variants are rejected.
- [x] **Step 2: Run the focused tests and confirm the current package configuration fails.**
...
- [x] **Step 6: Commit package metadata and tarball validation.**
